...
Git hook scripts are useful for a local pre-flight check when working on a
codebase. Issues in code such as missing semicolons, trailing whitespace,
...
- **Automatic formatting**: applying a code formatter will ensure consistent
...
- [codespell](https://github.com/codespell-project/codespell) is designed
  primarily for checking misspelled words in source code (backslash escapes are
  skipped), but it can be used with other files as well. Words that should be
  ignored are listed in `.codespell-ignore-words.txt`
...
- [markdownlint-cli2](https://github.com/DavidAnson/markdownlint-cli2)
  ...
  consistency in the output. Specific configuration is handled in
  `.markdownlint.yaml`.
...
and although it's in maintenance-only mode, it still works like a charm.
...
With this setup it's possible to define and enforce CI checks centrally,
