...
Although there's been support for
...
for a while, the process entailed setting environment variables and messing
with configuration options (i.e.
[setting ollama's context length](https://docs.ollama.com/context-length))
for better results.
...
and Codex with local or cloud models. No environment variables or messing
with config files needed.

The easiest way to set the adequate options is using the `--config` option.
